...
class EKF:
    ...
    def initialize(self, measurement):
        ...
        if measurement.sensor == Sensor.RADAR:
            self._x = np.array([measurement.data[0]*cos(measurement.data[1]),measurement.data[0]*sin(measurement.data[1]), 0, 0])
        else:
            self._x = np.array([measurement.data[0], measurement.data[1], 0, 0])
        self._P = np.array([[self.noise_acc_x_std **2, 0, 0, 0],
                           [0, self.noise_acc_y_std **2, 0, 0],
                           [0, 0, 1000, 0],
                           [0, 0, 0, 1000]])
    ...
    def predict(self, dt):
        ...
        self._F = np.array ([[1, 0, dt, 0],
                             ...
                             [0, 0, 1, 0],
                             [0, 0, 0, 1]])
        self._Q = np.array ([[dt*dt*dt*dt/4*self.noise_acc_x_std**2, 0, dt*dt*dt/2*self.noise_acc_x_std**2, 0],
               [0, dt*dt*dt*dt/4*self.noise_acc_y_std**2, 0, dt*dt*dt/2*self.noise_acc_y_std**2],
               [dt*dt*dt/2*self.noise_acc_x_std**2, 0, dt*dt*self.noise_acc_x_std**2, 0],
               [0, dt*dt*dt/2*self.noise_acc_y_std**2, 0,dt*dt*self.noise_acc_y_std**2]])
